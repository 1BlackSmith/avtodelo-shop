$(document).ready(function() {
		$('.product-detail-slider-image').fancybox({
      helpers : {
        media : {}
      }
    });
...
(function(window){
	...
	if (window.JCCatalogElementPreview)
		return;
	...
	window.JCCatalogElementPreview = function(arParams)
	{
		this.productType = 0;
	...
	window.JCCatalogElementPreview.prototype = {
...
*/
		...
		initConfig: function()
		{
			...
			this.sliderSlideCount = parseInt(this.params.CONFIG.SLIDER_SLIDE_COUNT, 10);
...
*/
			...
			else
			{
				...
				{
					var controls = this.product.slider.CONT.querySelectorAll('[data-go-to]');
...
*/
		...
		{
			var slides = this.node.imageContainer.querySelectorAll('img'),
				thumbs = this.product.slider.CONT.querySelectorAll('[data-go-to]');
			...
			thumbs[this.getItemIndex(this.eq(this.product.slider.ITEMS, index))] && BX.addClass(thumbs[this.getItemIndex(this.eq(this.product.slider.ITEMS, index))], 'active');
			...
			slides[this.getItemIndex(this.eq(this.product.slider.ITEMS, index))] && BX.addClass(slides[this.getItemIndex(this.eq(this.product.slider.ITEMS, index))].parentNode, 'active');
		},
...
*/
				...
				if (this.offers[index].SLIDER_COUNT > 0)
				{
					...
					for (i in this.offers[index].SLIDER)
					{
						if (this.offers[index].SLIDER.hasOwnProperty(i))
						{
							var slide = BX.create('span', {
								...
								children: [
									BX.create('IMG', {
										...
										attrs: {src: this.offers[index].SLIDER[i].SRC}
									})
								]
							});

							if (i > this.sliderSlideCount - 1 && this.offers[index].SLIDER_COUNT > this.sliderSlideCount)
							{
								var photoInfo = BX.message('SLIDER_MORE_MESSAGE');
								photoInfo = photoInfo.replace('#NUMBER#', this.offers[index].SLIDER_COUNT - this.sliderSlideCount);
							...
							this.node.imageContainer.appendChild(slide);
						}
					}
					...
					if (this.product.slider.CONT)
					{
						BX.cleanNode(this.product.slider.CONT);

						for (i in this.offers[index].SLIDER)
						{
							if (this.offers[index].SLIDER	.hasOwnProperty(i))
							{
								this.product.slider.CONT.appendChild(
									BX.create('span', {
										attrs: {
											'data-go-to': i,
								...
								// this.product.slider.CONT.appendChild(document.createTextNode(' '));
							}
						}

						this.product.slider.ITEMS = this.getEntities(this.product.slider.CONT, 'slider-control');
						this.product.slider.CONT.style.display = '';
					}
