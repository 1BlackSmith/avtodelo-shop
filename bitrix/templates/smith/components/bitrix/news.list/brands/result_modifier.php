...
// Разграничение показа брендов на разных сайтах
if (!function_exists('brands_filter')) {
	function brands_filter($section) 
	{
		$arDispay = array();

		$rsResult = CIBlockElement::GetProperty($section["IBLOCK_ID"], $section["ID"], "sort", "asc", array("CODE" => "DISPLAY"));
		while ($arItem = $rsResult->GetNext()) {
			$arDisplay[] = $arItem["VALUE_XML_ID"];
		}

		return in_array(SITE_ID, $arDisplay);
	}
}; unset($arDispay);
$arResult["ITEMS"] = array_filter($arResult["ITEMS"], "brands_filter");
