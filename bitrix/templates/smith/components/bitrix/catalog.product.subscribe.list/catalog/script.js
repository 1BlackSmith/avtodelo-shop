...
window.JCCatalogProductSubscribeList.prototype.hoverOn = function(event)
{
...
window.JCCatalogProductSubscribeList.prototype.hoverOff = function(event)
{
...
/*
				obData.props.className = (isCurrent ? 'checked' : '');
*/
				if (isCurrent)
				{
					BX.addClass(RowItems[i], 'checked');
		...
		BX.ajax({
			...
			onsuccess: BX.delegate(function (result) {
				if(result.success)
				{
					...
					this.delay = parseInt(this.deleteSubscribeBtn.attributes.delay.value);
					if(isNaN(this.delay))
						this.delay = 0;
	...
	window.JCCatalogProductSubscribeList.prototype.showWindowWithAnswer = function(answer)
	{
		...
		var messageBox = BX.create('div', {
			...
			children: [
				BX.create('div', {
					props:{
						className: 'mt-6 d-flex align-items-center fancybox-long'
					},
					children: [
						BX.create('div', {
							props:{
							...
							html: '<svg class="icon-svg" style="font-size: 3.125rem;"><use xlink:href="#svg-select-circle"></use></svg>',
						}),
						BX.create('div', {
							props:{
								className: 'd-block'
		...
		RS.Utils.popup(messageBox, 'window', {
			 title: BX.message('CPSL_SUBSCRIBE_TITLE_NEW')
		});
/*
