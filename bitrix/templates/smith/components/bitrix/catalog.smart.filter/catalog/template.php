...
*/?>
				...
				foreach ($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					if ($arItem['USING'] == 'Y')
						$boxSubClasses.= 'bx-filter-using ';
					?>
					<div class="<?=$colPropClasses?><?=$boxSubClasses?><?if ($arItem["DISPLAY_EXPANDED"] == "Y" && $arParams['FILTER_VIEW_MODE'] == 'VERTICAL'):?>bx-active<?endif?>"<?if(in_array($arItem['CODE'], $arHiddenProps)):?> style="display:none"<?endif?>>
						<span class="bx-filter-container-modef"></span>
						<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)">
...
*/?>
							...
							if ($isScrolable && count($arItem["VALUES"]) > 5)
							{
								?>
                                    </div>
								</div>
			...
			<?php endif; ?>
					<div class="bx-filter-block mt-1<?if(!isset($arResult["ELEMENT_COUNT"])) echo ' d-md-none';?>">
