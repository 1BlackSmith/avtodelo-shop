...
<head>
	...
	switch(SITE_ID) {
		case 's1':
			$asset->addString('<meta name="yandex-verification" content="58f60ded130bcc69" />');
			break;
		case 's2':
			$asset->addString('<meta name="yandex-verification" content="b2905370542e509d" />');
			$asset->addString('<meta name="wmail-verification" content="9dc2c9bc38e6d77d74a74ee83f3da475" />');
			break;
	}
	...
	$asset->addJs(SITE_DIR.'include/js/metrica.js');
	...
	$asset->addJs(SITE_DIR.'assets/styles/custom.js');
  	$asset->addJs(SITE_DIR.'theme_custom/func.js');
	$asset->addJs(SITE_DIR.'theme_custom/all.js');
  	$asset->addJs(SITE_DIR.'theme_custom/header.js');
 	$asset->addJs(SITE_DIR.'theme_custom/footer.js');
  	if ($APPLICATION->GetCurPage() == '/personal/order/make/') {
		$asset->addJs(SITE_DIR.'personal/order/make/autofill.js');
	}
	...
	$asset->addCss(SITE_DIR.'assets/styles/custom.css');
  	$asset->addCss(SITE_DIR.'theme_custom/all.css');
	$asset->addCss(SITE_DIR.'theme_custom/header.css');
  	$asset->addCss(SITE_DIR.'theme_custom/footer.css');
	if ($APPLICATION->GetCurPage() == '/company/staff/') {
		$asset->addCss(SITE_DIR.'theme_custom/staff.css');
	}
	if ($APPLICATION->GetCurPage() == '/') {
		$asset->addCss(SITE_DIR.'theme_custom/main-page.css');
	}
