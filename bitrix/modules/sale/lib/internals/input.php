...
class Date extends Base
{
	public static function getEditHtmlSingle($name, array $input, $value)
	{
		...
		$textAttributes = static::extractAttributes($input,
			array('DISABLED'=>$input['DISABLED_INPUT'], 'AUTOCOMPLETE'=>'on', 'AUTOFOCUS'=>'', 'READONLY'=>'',  'REQUIRED'=>''),
			array('FORM'=>1, 'LIST'=>1));
	...
	static function getSettings(array $input, $reload)
	{
		...
		return array(
			...
			'DISABLED_DATES' => array('TYPE' => 'Y/N', 'LABEL' => Loc::getMessage('INPUT_DATE_DISABLED_DATES')),
			// TODO min, max, step
		);
	}
}
