...
abstract class UserImportBase extends ImportBase
{
	 ...
	 * Типы покупателей
	 */
	const FIZ_PERSON_TYPE = 3;
	const UR_PERSON_TYPE  = 4;
	const IP_PERSON_TYPE  = 5;
	...
	public function getFieldsConfig($orgFormId, $userProps=array())
	{
		...
		*/
		...
		$config = $this->getConfigByType($orgFormId);
	 ...
	 * @return array
	 */
	public function getConfigByType($orgFormId)
	{
		static $config = null;

		if($config === null)
		{
			$r = \CSaleExport::GetList(array(), array("PERSON_TYPE_ID" => $orgFormId));
			while($ar = $r->Fetch())
			{
	...
	public function resolvePersonTypeId($fields)
	{
		...
		if ($this->isFiz())
			return self::FIZ_PERSON_TYPE;

		if ($this->isIp())
			return self::IP_PERSON_TYPE;

		if ($this->isUr())
			return self::UR_PERSON_TYPE;
	...
	public function registerUser($fields, &$arErrors)
	{
		...
		$profileFields = [
			'WORK_PHONE' => $fields["CONTACT"]["PHONE"],
		];

		if ($this->isUr() || $this->isIp()) {
			$profileFields['WORK_COMPANY'] = $fields["ITEM_NAME"];
			$profileFields['UF_CONTACT_PERSON'] = $fields["CONTACT"]["CONTACT_PERSON"];
			$profileFields['UF_STORES'] = array($fields["ADDRESS"]["PRESENTATION"]);
		}
		...
		$obUser = new \CUser;
		$obUser->Update($id, $profileFields, true);
