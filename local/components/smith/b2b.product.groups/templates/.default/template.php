...
else
{
	...
	foreach ($arResult['PRODUCTS'] as $arItems)
	{
		...
		if (!empty($arItems['SKU_ITEMS']) && !empty($arItems['SKU_ITEMS']["SKU_ELEMENTS"]))
		{
			...
			{
				$row->AddField("ACTION", '<a href="javascript:void(0)" onclick="'.$jsClick.'; BX.PreventDefault(); return false;">'.GetMessage('SPS_DELETE').'</a>');
			}
			...
			foreach ($arSkuResult["SKU_ELEMENTS"] as $val)
			{
				...
				$rowSku->AddField("ACTION", '<a class="select-sku">' . GetMessage('SPS_DELETE') . '</a>');
		...
		else
		{
			...
			if ($arItems['TYPE'] != 'S')
			{
				...
				$row->AddField("ACTION", '<a class="select-sku">' . GetMessage('SPS_DELETE') . '</a>');
			}
			else
			{
				$arActions[] = array(
	...
	<div class="adm-s-search-sidebar-container-left " style="width: 20%;padding-bottom: 28px; display: none;">
	...
	<div class="adm-s-search-content-container-right" style="width: 100%;">
		<div class="adm-s-content">
			<div class="adm-s-search-container" style="display: none;">
			...
			<div class="adm-s-search-query-settings-container" style="display: none">
			...
			<form name="<?= $tableId ?>_find_form" method="GET" action="<? echo $APPLICATION->GetCurPage() ?>?" accept-charset="<? echo LANG_CHARSET; ?>" id="<?= $tableId ?>_form" style="display: none">
			...
			<div class="adm-navchain" style="vertical-align: middle; margin-left: 0; display: none" id="<?= $tableId ?>_breadcrumbs">
