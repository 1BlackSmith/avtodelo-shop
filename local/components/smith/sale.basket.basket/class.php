...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	protected function includeModules()
	{
		...
		if (!Loader::includeModule('smith.bonusengine'))
		{
			$success = false;
			ShowError(Loc::getMessage('SALE_MODULE_NOT_INSTALL'));
		}
	...
	{
		$this->loadGifts($this->getCouponInfo());
		//$this->addGiftsInBasket();
		...
		$this->arResult['USER_POINTS'] = BasketTable::roundPrecision($arUserAccount['CURRENT_BUDGET']);
		$this->arResult['DISABLE_BONUSES'] = $userId === null || $arUserAccount['CURRENT_BUDGET'] == 0 ? 'Y' : false;

		$basketBonuses = BasketTable::getBonuses(array('USER_ID' => $userId));
		$this->arResult['BASKET_BONUSES'] = BasketTable::roundPrecision($basketBonuses);
		$this->arResult['DEBITED_BONUSES'] = $this->arResult['BASKET_BONUSES'] ? true : false;

		if ($this->getBasketBasePrice() < 1200) {
			$this->arResult['SBB_FREE_DELIVERY_OFFER'] = true;
		} else {
			$this->arResult['SBB_FREE_DELIVERY'] = true;
		}
	...
	protected function getUserId()
	{
		...
		$this->manager = Manager::getByID($userId);
		if ($this->clientId) {
	...
	protected function getBasketItemsRawArray()
	{
		...
		while ($basketItem = $basketItemsResult->fetch())
		{
			...
			$basketItem['PRICE'] = (float)$basketItem['PRICE'];
			$basketItem['PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['PRICE'], "RUB", false);

			$basketItem['FULL_PRICE'] = (float)$basketItem['BASE_PRICE'];
			$basketItem['FULL_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['FULL_PRICE'], "RUB", false);

			$basketItem['DISCOUNT_PRICE'] = (float)$basketItem['DISCOUNT_PRICE'];
			$basketItem['DISCOUNT_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['DISCOUNT_PRICE'], "RUB", false);

			$basketItem['SUM_VALUE'] = $basketItem['PRICE'] * $basketItem['QUANTITY'];
			$basketItem['SUM'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_VALUE'], "RUB", false);

			$basketItem['SUM_FULL_PRICE'] = $basketItem['FULL_PRICE'] * $basketItem['QUANTITY'];
			$basketItem['SUM_FULL_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_FULL_PRICE'], "RUB", false);

			$basketItem['SUM_DISCOUNT_PRICE'] = $basketItem['DISCOUNT_PRICE'] * $basketItem['QUANTITY'];
			$basketItem['SUM_DISCOUNT_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_DISCOUNT_PRICE'], "RUB", false);
	...
	protected function processBasketItem(Sale\BasketItem $item)
	{
		...
		/**
		 * Изменение цен в соответствии с ценами в корзине
		 */
		...
		if ($item = $basket->getItemById($basketItem['ID'])) {
			$basketItem['CUSTOM_PRICE'] = 'Y';
			$basketItem['PRICE'] = $item->getField('PRICE');
		...
		$basketItem['PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['PRICE'], "RUB", true);

		$basketItem['FULL_PRICE'] = $basketItem['BASE_PRICE'];
		$basketItem['FULL_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['FULL_PRICE'], "RUB", true);

		$basketItem['DISCOUNT_PRICE'] = $basketItem['DISCOUNT_PRICE'];
		$basketItem['DISCOUNT_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['DISCOUNT_PRICE'], "RUB", true);

		$basketItem['SUM_VALUE'] = $basketItem['PRICE'] * $basketItem['QUANTITY'];
		$basketItem['SUM'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_VALUE'], "RUB", true);

		$basketItem['SUM_FULL_PRICE'] = $basketItem['FULL_PRICE'] * $basketItem['QUANTITY'];
		$basketItem['SUM_FULL_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_FULL_PRICE'], "RUB", true);

		$basketItem['SUM_DISCOUNT_PRICE'] = $basketItem['DISCOUNT_PRICE'] * $basketItem['QUANTITY'];
		$basketItem['SUM_DISCOUNT_PRICE_FORMATED'] = CCurrencyLang::CurrencyFormat($basketItem['SUM_DISCOUNT_PRICE'], "RUB", true);

		$basketItem['PRICE_VAT_VALUE'] = $basketItem['VAT_VALUE']
			= ($basketItem['PRICE'] * $basketItem['QUANTITY'] / ($basketItem['VAT_RATE'] + 1)) * $basketItem['VAT_RATE'] / $basketItem['QUANTITY'];

		$basketItem['DISCOUNT_PRICE_PERCENT'] = 100 - round($basketItem['PRICE'] / $basketItem['BASE_PRICE'] * 100);
	...
	protected function getBasketTotal()
	{
		...
		if ($this->isFastLoadRequest())
		{
			...
			foreach ($this->basketItems as $basketItem)
			{
				if ($basketItem['CAN_BUY'] === 'Y' && $basketItem['DELAY'] !== 'Y')
				{
					$basketPrice += $basketItem['PRICE'] * $basketItem['QUANTITY'];
		...
		$userId = $this->getUserId();
		$basketBonuses = BasketTable::getBonuses(array('USER_ID' => $userId));
		$result['BASKET_BONUSES'] = BasketTable::roundPrecision($basketBonuses);
		$result['DEBITED_BONUSES'] = $result['BASKET_BONUSES'] ? true : false;

		$arUserAccount = CSaleUserAccount::GetByUserID($userId, 'RUB');
		$result['USER_POINTS'] = BasketTable::roundPrecision($arUserAccount['CURRENT_BUDGET']);

		$result['DISABLE_BONUSES'] = $userId === null || $arUserAccount['CURRENT_BUDGET'] == 0 ? 'Y' : false;

		$result['PRICE_WITHOUT_DISCOUNT'] = CCurrencyLang::CurrencyFormat($basketBasePrice, $siteCurrency, true);
		$result['DISCOUNT_PRICE_ALL'] = $basketBasePrice - $basketPrice;
		...
		else
		{
			...
			if ($found)
			{
				if (!empty($coupon))
				{
					...
					//$res = DiscountCouponsManager::setApply($coupon, array('BASKET' => $this->getBasketItemsArray()));
	...
	protected function getBasketBasePrice()
	{
		$basket = $this->getBasketStorage()->getBasket();
		$price = 0;
		...
		foreach ($basket as $basketItem) {
			...
			$price += $basketItem->getBasePrice() * $quantity;
		}
	...
	protected function loadGifts($couponList)
	{
	...
	protected function addGiftsInBasket()
	{
		$basket = &$this->getBasketStorage()->getBasket();

		foreach ($this->arGiftIds as $productId) {
