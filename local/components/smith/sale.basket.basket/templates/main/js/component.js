;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		toggleFilter: function(event)
		{
			...
			for (var i = 0; i < entities.length; i++)
			{
				...
				{
					BX.addClass(entities[i], 'btn-secondary');
					BX.removeClass(entities[i], 'btn-outline-secondary');
				}
				else if (BX.hasClass(entities[i], 'btn-secondary'))
				{
					BX.addClass(entities[i], 'btn-outline-secondary');
					BX.removeClass(entities[i], 'btn-secondary');
				}
				/* CUSTOM */
			}
		...
		checkStickyHeaders: function()
		{
			...
			/* CUSTOM */
			var stickyHeader  = document.querySelector('.js-compact-header.is-fixed > .l-compact-header');
			if (stickyHeader) {
		...
		{
			if (this.result.EVENT_ONCHANGE_ON_START === 'Y' || this.result.allSum == 0)
			{
		...
		showItemsCount: function()
		{
			...
			{
				itemCountNode.innerHTML = BX.message('SBB_IN_BASKET') + ' <span>' + this.result.BASKET_ITEMS_COUNT + '</span>';
		...
		bindTotalEvents: function(node)
		{
			...
			BX.bind(this.getEntity(node, 'basket-bonuses-button'), 'change', BX.proxy(this.calculateBonusesAction, this));
		...
		// @pool
		calculateBonusesAction: function(event)
		{
			var target = BX.getEventTarget(event);
			if (target) {
				this.actionPool.calculateBonuses(target.checked ? true : false);
			}
		},
		...
		sendRequest: function(action, data)
		{
			...
			// Если выбран пересчет бонусов
			var totalNode = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-total-block'),
				basketBonusesBtn = this.getEntity(totalNode, 'basket-bonuses-button');

			if (basketBonusesBtn.checked) {
				data.basket.bonuses_recalculation = 'Y';
			} else {
				data.basket.bonuses_recalculation = 'N';
			}
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					if (this.result.DEBITED_BONUSES) 
					{
						this.result.TOTAL_RENDER_DATA.DEBITED_BONUSES = this.result.DEBITED_BONUSES;
						this.result.TOTAL_RENDER_DATA.BASKET_BONUSES = this.result.BASKET_BONUSES;
					}
					if (this.result.DISABLE_BONUSES) {
						this.result.TOTAL_RENDER_DATA.DISABLE_BONUSES = this.result.DISABLE_BONUSES;
					}
                    this.editTotal();
        ...
        updateBasket: function () {
            var products = this.result.BASKET_ITEM_RENDER_DATA;
            ...
            if (products.length > 0)
            {
                ...
                Basket.inbasket(productsIds, true);
            } 
            else
            {
                if (window.Basket)
		...
		checkTotalAnimation: function(totalData)
		{
			if (this.result && this.result.TOTAL_RENDER_DATA && parseFloat(this.result.TOTAL_RENDER_DATA.PRICE) > parseFloat(totalData.PRICE))
			{
				...
				totalData.DEBITED_BONUSES = this.result.DEBITED_BONUSES;
		...
		showCustomErrors: function ()
		{
			var customErrorsBlock = this.getEntity(this.getCacheNode(this.ids.basketRoot), 'basket-custom-errors');

			if (BX.type.isDomNode(customErrorsBlock))
			{
				var errorNode = this.getEntity(customErrorsBlock, 'basket-custom-errors-content');
				...
				if (BX.type.isDomNode(errorNode))
				{
					...
					if (
						BX.type.isArray(this.result.CUSTOM_ERROR_MESSAGES) &&
						this.result.CUSTOM_ERROR_MESSAGES.length > 0
					) {

						for (var i in this.result.CUSTOM_ERROR_MESSAGES)
						{
							if (this.result.CUSTOM_ERROR_MESSAGES.hasOwnProperty(i))
							{
								errorText += this.result.CUSTOM_ERROR_MESSAGES[i];
							}
						}
					}
					...
					if (errorText)
					{
						...
						customErrorsBlock.style.display = 'block';
					}
					else
					{
						errorNode.innerHTML = '';
						customErrorsBlock.style.display = 'none';
					}
				}
			}
		},
		...
		selectClient: function(event)
		{
			var target = BX.getEventTarget(event);
			if (target) {
				this.actionPool.selectClient(target.value);
			}
		},
		...
		clearAction: function () {
			for (var id in this.items)
			{
				if (this.items.hasOwnProperty(id) && this.isItemAvailable(id))
				{
					this.actionPool.deleteItem(id);

					this.items[id].SHOW_LOADING = true;

					if (this.params.SHOW_RESTORE === 'Y' && this.isItemAvailable(id))
					{
						this.items[id].SHOW_RESTORE = true;
					}

					this.redrawBasketItemNode(id);
				}
			}
		},
	...
	BX.Sale.BasketComponentCopy = BX.clone(BX.Sale.BasketComponent, true);
})();
