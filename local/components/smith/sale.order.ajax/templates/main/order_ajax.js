		...
		editRegistrationForm: function(authContent)
		{
			...
			if (this.result.AUTH.captcha_registration == 'Y')
			{
                authFormNodes.push(BX.create('DIV', {
                    props: {className: 'bx-authform-formgroup-container captcha-wrap mb-4'},
                    children: [
                        BX.create('DIV', {
...
*/
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			switch (propertyType)
			{
				...
				case 'NUMBER':
					...
					break;
				case 'STORES':
					this.insertStoresProperty(property, propsItemNode, disabled);
					break;
			}
		...
		insertStoresProperty: function(property, propsItemNode, disabled)
		{
			var prop, inputs, values, i, propContainer;
		...
		editTotalBlock: function()
		{
			...
			if (total.DEDUCTED_POINTS)
			{
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_DEDUCTED_POINTS') + ':', String(total.DEDUCTED_POINTS), params));
			}

			if (total.AWARDED_POINTS)
			{
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_AWARDED_POINTS') + ':', String(total.AWARDED_POINTS), params));
			}
			...
			else
			{
				if (parseFloat(total.DELIVERY_PRICE) === 0)
				{
					...
					if (curDelivery.NAME == BX.message('PICKUP_DEFAULT')) 
					{
						deliveryValue = BX.message('PICKUP_DEFAULT_SMALL');
						params.free = false;
					}
				}
				else
