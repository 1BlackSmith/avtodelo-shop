...
use Smith\B2B\Handlers\InputTypes;
...
if (!$arResult['ALLOW_ORDER'])
{
	...
	if (isset($arParams['MIN_PRICE_REDIRECT']) && $arParams['MIN_PRICE_REDIRECT'] != 'N' && !$USER->IsAdmin())
	{
		LocalRedirect($arParams['PATH_TO_BASKET'], true);
	}
}
...
if ($arResult['SMITH_PERSONE_TYPE'] === 3)
{
...
else 
{
	if ($useDefaultMessages || !isset($arParams['MESS_BUYER_BLOCK_NAME']))
	{
		$arParams['MESS_BUYER_BLOCK_NAME'] = Loc::getMessage('B2B_BUYER_BLOCK_NAME_DEFAULT');
	}
}
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<?php if (!$arResult['ALLOW_ORDER']):?>
			<div class="alert alert-danger alert-basket-custom-errors">
				<svg class="icon-svg"><use xlink:href="#svg-error"></use></svg>
				<span>
					<?=$arResult['ALLOW_ORDER_ERROR_MESSAGE']?>
				</span>
			</div>
		<?php endif; ?>
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-lg-9 bx-soa">
				...
				<? if ($arParams['BASKET_POSITION'] === 'before'): ?>
					...
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<div class="row align-items-center">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-icon"></span><?=$arParams['MESS_BASKET_BLOCK_NAME']?>
								</h2>
						...
						<div class="row align-items-center">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-icon">
									<svg class="icon-svg is-completed"><use xlink:href="#svg-select-circle"></use></svg>
									<svg class="icon-svg"><use xlink:href="#svg-order-pin-small"></use></svg>
								</span>
					...
					<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<div class="row align-items-center">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-icon">
										<svg class="icon-svg"><use xlink:href="#svg-order-card"></use></svg>
									</span>
					...
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery ? 'style="display:none"' : '')?>>
						<div class="bx-soa-section-title-container">
							<div class="row align-items-center">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-icon">
										<svg class="icon-svg"><use xlink:href="#svg-order-delivery"></use></svg>
									</span>
					...
					<div id="bx-soa-pickup" data-visited="false" class="bx-soa-section" style="display:none">
						<div class="bx-soa-section-title-container">
							<div class="row align-items-center">
								...
								<div class="col-xs-12 col-sm-3 text-right pr-0"><a href="" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a></div>
							</div>
						</div>
						<div class="bx-soa-section-content container-fluid"></div>
					</div>
				...
				<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
					<div class="bx-soa-section-title-container">
						<div class="row align-items-center">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-icon">
									<svg class="icon-svg"><use xlink:href="#svg-order-user"></use></svg>
									<svg class="icon-svg is-completed"><use xlink:href="#svg-select-circle"></use></svg>
								</span>
				...
				<script>
					BX.ready(function() {
					   	var properties = document.getElementById('bx-soa-properties');
					   	properties.addEventListener('DOMSubtreeModified', function() {
					    	if (!$(properties).find('input').length) return;

						    var result = new BX.MaskedInput({
						        mask: '+7 (999) 999-99-99',
						        input: BX('soa-property-22'),
						        placeholder: '_'
						    });

						    result.setValue("<?= $arResult['USER_PHONE'] ?>");
						});
					});
				</script>
					...
					<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active">
						<div class="bx-soa-section-title-container">
							<div class="row align-items-center">
								<h2 class="bx-soa-section-title col-sm-9">
									<span class="bx-soa-section-title-icon">
										<svg class="icon-svg"><use xlink:href="#svg-order-cart"></use></svg>
										<svg class="icon-svg is-completed"><use xlink:href="#svg-select-circle"></use></svg>
									</span>
									<?=$arParams['MESS_BASKET_BLOCK_NAME']?>
								</h2>
								<div class="col-xs-12 col-sm-3 text-right pr-0"><a href="javascript:void(0)" class="bx-soa-editstep"><?=$arParams['MESS_EDIT']?></a></div>
							</div>
						</div>
						<div class="bx-soa-section-content container-fluid"></div>
					</div>
				...
				<div id="bx-soa-orderSave" class="bx-soa-order-save">
					...
					<a href="javascript:void(0)" class="mt-6 btn btn-primary d-none d-sm-block<?php if (!$arResult['ALLOW_ORDER']): ?> disabled<?php endif; ?>" data-save-button="true">
						<?=$arParams['MESS_ORDER']?>
					</a>
				</div>
