...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	const INDIVIDUAL_PERSON_TYPE = 'INDIVIDUAL';
	const COMPANY_PERSON_TYPE = 'COMPANY';
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		$arParams['USE_PRELOAD'] = 'N';//$arParams['USE_PRELOAD'] === 'N' ? 'N' : 'Y';
	...
	protected function getOrderPropFormatted($arProperty, &$arDeleteFieldLocation = array())
	{
		...
		switch ($arProperty['TYPE'])
		{
			...
			case 'STORES': self::formatStores($arProperty); break;
		}
	...
	public static function formatStores(array &$arProperty)
	{
		$curVal = $arProperty['VALUE'];
	...
	protected function obtainTotal()
	{
		...
		$basketPoints = BasketTable::getBonuses(array('USER_ID' => $this->order->getUserId()));
		if ($basketPoints) {
			$arResult['DEDUCTED_POINTS'] = BasketTable::roundPrecision($basketPoints);
		}

		$awardedPoints = SBHandlers::getAwardedSum($this->order);
		if ($awardedPoints) {
			$arResult['AWARDED_POINTS'] = BasketTable::roundPrecision($awardedPoints);
		}
	}
	...
	protected function initPersonType(Order $order)
	{
		...
		if (Rights::isCompany($this->userId))
		{
			$personTypeCode = self::COMPANY_PERSON_TYPE;
		} 
		else 
		{
			$personTypeCode = self::INDIVIDUAL_PERSON_TYPE;
		}

		foreach ($personTypes as $personType) {
			if ($personTypeCode === $personType['CODE'])
			{
				$personTypeId = intval($personType['ID']);
				$order->setPersonTypeId($personTypeId);
				$this->arResult['SMITH_PERSONE_TYPE'] = $personTypeId;
	...
	protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
	{
		...
		if ($sumToSpend > 0)
		{
			...
			$arResult['CURRENT_BUDGET_FORMATED'] = $arResult['USER_ACCOUNT']['CURRENT_BUDGET'];
		}
		else
		{
	...
	protected function refreshOrderAjaxAction()
	{
		...
		{
			$this->order = $this->createOrder($this->userId ? $this->userId : CSaleUser::GetAnonymousUserID());
	...
	protected function saveOrderAjaxAction()
	{
		...
		if ($this->checkSession)
		{
			...
			{
				$userId = $this->userId ? $this->userId : CSaleUser::GetAnonymousUserID();
			}
			...
			if ($isActiveUser && empty($this->arResult['ERROR']))
			{
		    	...
		    	$coupons = DiscountCouponsManager::get(false, array(), true, true);
	...
	protected function getBasketItemsArray()
    {
		...
		{
			$basketItems[$item->getId()] = $item->getFieldValues();
		}
	...
	protected function getJsDataResult()
	{
		...
		)
		{
			...
			if (!empty($lastOrder))
			{
				...
				if (!empty($lastOrder['PAY_SYSTEM_ID']) && $payment = $this->getExternalPayment($this->order))
				{
		...
		$result['TOTAL'] = array(
			...
			'DEDUCTED_POINTS' => $arResult["DEDUCTED_POINTS"],
			'AWARDED_POINTS' => $arResult["AWARDED_POINTS"]
		);
	...
	protected function obtainUserId()
	{
		global $APPLICATION, $USER;

		if (Manager::getByID($USER)) {
			...
			$APPLICATION->set_cookie("B2B_CLIENT_ID", 0, 0);
			$this->userId = $USER->GetID();
		}
	}
	...
	protected function showOrderAction()
	{
		...
		if (!empty($arOrder) && ($order->getUserId() == $this->userId || $checkedBySession))
		{
