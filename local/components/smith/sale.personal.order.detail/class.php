...
class CBitrixPersonalOrderDetailComponent extends CBitrixComponent
{
	...
	/**
	 * Function gets user points info from database, no cache is used for it
	 * @return void
	 */
	// @obtainPoints
	protected function obtainDataPoints()
	{	
		$order = $this->order;

		if (Loader::includeModule("smith.bonusengine")) 
		{
			$this->dbResult["DEDUCTED_POINTS"] = BasketTable::getBonuses(array('ORDER_ID' => $order->getId()));

			if (!$this->dbResult["DEDUCTED_POINTS"] && $this->paymentsIsPayed()) {
				$this->dbResult["AWARDED_POINTS"] = SBHandlers::getAwardedSum($order);
			}
		}
	}

	private function paymentsIsPayed()
	{
		foreach ($this->dbResult['PAYMENT'] as &$payment)
		{
			if ($payment["PAID"] != "Y")
	...
	/**
	 * Function formats points info in arResult
	 * @return void
	 */
	// protected function formatResultPoints()
	// {
	// 	$arResult =& $this->arResult;
