...
// 	//LocalRedirect( $backUrl );
// }
if (is_set($_REQUEST[$arParams['ACTION_CHANGE_TEMPLATE']])) { //_______ change template
	$val = htmlspecialchars($_REQUEST[$arParams['ACTION_CHANGE_TEMPLATE']]);
	$_SESSION[$com_uniq_]['ALFASORTER_CTEMPLATE'] = $val;
	//LocalRedirect( $backUrl );
} 
if (is_set($_REQUEST[$arParams['ACTION_CHANGE_SORT']])) { //_______ change sort type
	$val = htmlspecialchars($_REQUEST[$arParams['ACTION_CHANGE_SORT']]);
	$_SESSION[$com_uniq_]['ALFASORTER_CSORT'] = $val;
	//LocalRedirect( $backUrl );
} 
if (is_set($_REQUEST[$arParams['ACTION_CHANGE_OUTPUT']])) { //_______ change elements on page
	$val = htmlspecialchars($_REQUEST[$arParams['ACTION_CHANGE_OUTPUT']]);
	$_SESSION[$com_uniq_]['ALFASORTER_COUTPUT'] = $val;
	//LocalRedirect( $backUrl );
}
...
if ($arParams['ALFA_CHOSE_TEMPLATES_SHOW'] == 'Y') {
	...
	for ($i = 0; $i < $arParams['ALFA_CNT_TEMPLATES']; $i++) {
		$addParam = $arParams['ACTION_CHANGE_TEMPLATE'].'='.$arParams['ALFA_CNT_TEMPLATES_NAME_'.$i];
		$arResult['CTEMPLATE'][] = array(
			'NAME_LANG' => $arParams['ALFA_CNT_TEMPLATES_'.$i],
			'VALUE' => $arParams['ALFA_CNT_TEMPLATES_NAME_'.$i],
			'USING' => 'N',
			'URL' => $APPLICATION->GetCurPageParam($addParam,array($arParams['ACTION_CHANGE_TEMPLATE'])),
		);
	}
}
...
if ($arParams['ALFA_SORT_BY_SHOW'] == 'Y') {
	foreach ($arParams['ALFA_SORT_BY_NAME'] as $key2 => $sort) {
		$addParam1 = $arParams['ACTION_CHANGE_SORT'].'='.$sort.'_asc';
		$addParam2 = $arParams['ACTION_CHANGE_SORT'].'='.$sort.'_desc';
		...
		$arResult['CSORTING'][] = array(
			...
			'DIRECTION' => 'asc',
			'USING' => 'N',
			'URL' => $APPLICATION->GetCurPageParam($addParam1,array($arParams['ACTION_CHANGE_SORT'])),
			'URL2' => $APPLICATION->GetCurPageParam($addParam2,array($arParams['ACTION_CHANGE_SORT'])),
		);
		...
		$arResult['CSORTING'][] = array(
			...
			'DIRECTION' => 'desc',
			'USING' => 'N',
			'URL' => $APPLICATION->GetCurPageParam($addParam2,array($arParams['ACTION_CHANGE_SORT'])),
			'URL2' => $APPLICATION->GetCurPageParam($addParam1,array($arParams['ACTION_CHANGE_SORT'])),
		);
	}
}
...
if ($arParams['ALFA_OUTPUT_OF_SHOW'] == 'Y') {
	...
	foreach ($arParams['ALFA_OUTPUT_OF'] as $key3 => $output) {
		if ($output != '') {
			$addParam = $arParams['ACTION_CHANGE_OUTPUT'].'='.$output;
			$arResult['COUTPUT'][] = array(
				'NAME_LANG' => $output,
				'VALUE' => $output,
				'USING' => 'N',
				'URL' => $APPLICATION->GetCurPageParam($addParam,array($arParams['ACTION_CHANGE_OUTPUT'])),
			);
		}
	}
    
	if ($arParams['ALFA_OUTPUT_OF_SHOW_ALL'] == 'Y') {
		$addParam = $arParams['ACTION_CHANGE_OUTPUT'].'=1000000';
		$arResult['COUTPUT'][] = array(
