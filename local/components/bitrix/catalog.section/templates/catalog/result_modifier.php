...
$component->arParams['MESS_ERROR_SECTION_EMPTY'] = $component->arParams['MESS_ERROR_SECTION_EMPTY'] ? $component->arParams['MESS_ERROR_SECTION_EMPTY'] : Loc::getMessage('RS_MM_BCS_CATALOG_ERROR_EMPTY_ITEMS');
...
if (Loader::includeModule('smith.tuning')) {
    \Smith\Tuning\PriceModList::modificateAllItems($arResult);
    foreach ($arResult['ITEMS'] as &$item) {
    	if (is_set($arResult['PRICES'][$item['ID']])) {
    		$arPrices = $arResult['PRICES'][$item['ID']];
    	}

    	if (is_set($item['PRICES'])) {
    		$arPrices = $item['PRICES'];
    	}

    	\Smith\Tuning\PriceModList::modificateItem($item, $arPrices);
    }
}