...
<div class="product-detail-price" data-entity="price">
    <div class="text-nowrap">
        ...
        {
            ?>
            <span class="product-cat-price-old" id="<?=$mainId.'_price_'.$price['PRICE_TYPE_ID']?>" style="display: <?=($showDiscount ? '' : 'none')?>;" data-entity="price-full">
    ...
    {
        ?>
        <div class="product-cat-price-economy" id="<?=$mainId.'_price_discount_'.$price['PRICE_TYPE_ID']?>"  style="display: <?=($showDiscount ? '' : 'none')?>;" data-entity="price-discount">
            <?
            if ($showDiscount)
            {
                echo Loc::getMessage('CT_BCE_CATALOG_ECONOMY_INFO2', array('#ECONOMY#' => $price['PERCENT']));
            }
            ?>
        </div>
        <?
