...
// if (is_array($actualItem['PROPERTIES']['MORE_PHOTO']['VALUE'])) {
// 	$arImg = "";
// 	foreach ($actualItem['PROPERTIES']['MORE_PHOTO']['VALUE'] as $key => $img) {
// 		$arImg .= ($key === 0) ? $img : ",$img";
// 	}

// 	$rsFile = CFile::GetList(array('ID' => 'desc'), array('@ID' => $arImg));
...
// 		);
//   }
// } elseif ($actualItem['PROPERTIES']['MORE_PHOTO']['VALUE'] !== false) {
// 	$rsFile = CFile::GetByID($actualItem['PROPERTIES']['MORE_PHOTO']['VALUE']);
// 	$arFile = $rsFile->Fetch();
...
//fix prices
if (Loader::includeModule('smith.tuning')) {
	$priceMod = new \Smith\Tuning\PriceMod($arResult);
	$priceMod->refreshCurPrice();
}

$selectedPrice = $arResult['ITEM_PRICE_SELECTED'];
$arCurPrice = &$arResult['ITEM_PRICES'][$selectedPrice];
/* Бонусы */
if (Loader::includeModule('smith.bonusengine')) {
	function setBonuses(&$arPrice, $bonus) 
	{
		$arPrice['PRICE_BONUS_PERCENT'] = $bonus;
		$arPrice['PRICE_BONUS'] = $arPrice['PRICE'] * $bonus / 100;
		$arPrice['RATIO_BONUS'] = $arPrice['PRICE_BONUS'];
		$arPrice['PRICE_BONUS_ROUNDED'] = BasketTable::roundPrecision($arPrice['PRICE_BONUS']);
		$arPrice['PRINT_RATIO_BONUS'] = CurrencyFormat($arPrice['PRICE_BONUS'], 'RUB');
	}

	if ($userRate = SBHandlers::getUserRate($USER->GetID())) {
		$bonus = $arResult['PROPERTIES'][$userRate]['VALUE'];

		setBonuses($arCurPrice, $bonus);

		foreach ($arResult['ITEM_ALL_PRICES'][0]['PRICES'] as &$arPrice) {
			setBonuses($arPrice, $bonus);
		}

		$arParams['SHOW_CASHBACK'] = 'Y';
	}
}
...
// Отображение складов для заданных групп
$arParams['USE_STORE'] = count($res) ? 'Y' : 'N';
