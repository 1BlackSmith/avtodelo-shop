		...
		else
		{
			...
			else
			{
				...
				if($arResult["USER_ACTIVE"] && $arResult["USER_ACTIVE"]["ACTIVE"] === "Y")
				{
					...
					/* Отправка писем после подтверждения регистрации */
					$siteId = (defined(SITE_ID)) ? SITE_ID : $arResult["USER_ACTIVE"]["LID"];
					...
					$arFields = array(
					    "USER_ID"     => $arResult["USER_ACTIVE"]["ID"],
					    "LOGIN"       => $arResult["USER_ACTIVE"]["LOGIN"],
					    "EMAIL"       => $arResult["USER_ACTIVE"]["EMAIL"],
					    "NAME"        => $arResult["USER_ACTIVE"]["NAME"],
					    "LAST_NAME"   => $arResult["USER_ACTIVE"]["LAST_NAME"],
					    "SITE_NAME"   => $siteName,
					    "SERVER_NAME" => $_SERVER["SERVER_NAME"],
					);

					CEvent::SendImmediate("AFTER_NEW_USER_CONFIRM", $siteId, $arFields);
				}
				else
				{
