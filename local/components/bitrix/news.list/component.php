...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	while($obElement = $rsElement->GetNextElement())
	{
		...
		// if (is_set($arItem['PROPERTIES']['SITE']) && !in_array(SITE_ID, $arItem['PROPERTIES']['SITE']['VALUE_XML_ID']))
		// 	continue;

		// if (is_set($arItem['PROPERTIES']['DISPLAY']) && !in_array(SITE_ID, $arItem['PROPERTIES']['DISPLAY']['VALUE_XML_ID']))
		// 	continue;
