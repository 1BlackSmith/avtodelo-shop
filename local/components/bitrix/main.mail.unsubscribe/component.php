...
// Отписка пользователя с помощью функций CSubscription
if (isset($_REQUEST['unsub'])) {
  $arResult['USER_CONFIRM_DATA']['ID'] = explode(".", $_REQUEST['unsub'])[0];
  $arResult['USER_CONFIRM_DATA']['EMAIL_HASH'] = explode(".", $_REQUEST['unsub'])[1];
  
  $rsUser = CUser::GetByID($arResult['USER_CONFIRM_DATA']['ID']);
  if ($arUser = $rsUser->Fetch()) {
    // Проверка хеша
    if (md5($arUser['NAME']."".strtolower($arUser['EMAIL'])) === $arResult['USER_CONFIRM_DATA']['EMAIL_HASH']) {
      CModule::IncludeModule('subscribe');
      $rsSubscr = CSubscription::GetList(
        array("ID" => "ASC"),
        array("EMAIL" => $arUser['EMAIL'])
      );
      
      while($arSubscr = $rsSubscr->Fetch()) {
        CSubscription::Delete($arSubscr["ID"]);
      }
    }
  }
}
